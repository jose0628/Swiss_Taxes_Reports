...
def generate_reports(
    ...
    if all_years:
        ...
        tx_file = OUTPUT_DIR / f"bitcoin_all_years_transactions{price_suffix}.xlsx"
        df_tx.to_excel(tx_file, index=False)
        print(f"Bitcoin all-years transactions: {len(df_tx)} rows -> {tx_file}")
        ...
        if not year:
            ...
            summary_file = OUTPUT_DIR / f"bitcoin_all_years_summary.xlsx"
            summary_df.to_excel(summary_file, index=False)
            print(f"Bitcoin all-years summary: {len(summary_df)} rows -> {summary_file}")
        else:
            print(f"Bitcoin: {len(year_transactions)} transactions for year {year}")
