#!/usr/bin/env python3
"""
Generate yearly Litecoin transaction reports.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate yearly Litecoin transaction reports."
    )
    parser.add_argument(
        "--year",
        type=int,
...
def fetch_litecoin_transactions(wallet: str) -> List[dict]:
    """Fetch Litecoin transactions using litecoinspace.org API."""
...
def parse_litecoin_tx(wallet: str, tx: dict) -> dict:
    """Parse a Litecoin transaction into a record."""
    status = tx.get("status", {})
    block_time = status.get("block_time")
    ...
    amount_ltc = abs(balance_change) / 1e8
    direction = "in" if balance_change >= 0 else "out"
    fee_ltc = int(tx.get("fee", 0) or 0) / 1e8 if spent else 0.0
    ...
    return {
        ...
        "direction": direction,
        "amount_coin": amount_ltc,
        "signed_amount_coin": balance_change / 1e8,
        "fee_coin": fee_ltc if balance_change < 0 else 0.0,
        "price_usd": None,
        "amount_usd": None,
        "fee_usd": None,
        "from_address": "",
...
def generate_reports(
    ...
    """Generate Litecoin transaction reports."""
    ...
    litecoin_wallets = wallet_addresses.get("litecoin", []) or []
    
    if not litecoin_wallets:
        print("No Litecoin wallets configured.")
        return
    ...
    if all_years:
        ...
        for wallet in litecoin_wallets:
            txs = fetch_litecoin_transactions(wallet)
            wallet_data[wallet] = txs
            
            for tx in txs:
                record = parse_litecoin_tx(wallet, tx)
        ...
        if include_prices:
            for record in year_transactions:
                px = get_price_usd("litecoin", record["timestamp_utc"], coingecko_demo_key, coingecko_pro_key)
                record["price_usd"] = px
        ...
        if include_prices:
            ...
            print(f"Total Litecoin transactions (USD): ${total_tx:,.2f}")
        else:
            total_tx = sum(r["amount_coin"] for r in year_transactions)
            print(f"Total Litecoin transactions: {total_tx:.8f} LTC")
    else:
        if year is None:
        ...
        if not df_tx.empty:
            df_tx = df_tx.sort_values("timestamp_utc").reset_index(drop=True)
        price_suffix = "_usd" if include_prices else ""
        tx_file = OUTPUT_DIR / f"litecoin_{year}_transactions{price_suffix}.xlsx"
        df_tx.to_excel(tx_file, index=False)
        print(f"Litecoin transactions: {len(df_tx)} rows -> {tx_file}")
        ...
        summary_df = pd.DataFrame(summary_rows)
        summary_file = OUTPUT_DIR / f"litecoin_{year}_summary.xlsx"
        summary_df.to_excel(summary_file, index=False)
        print(f"Litecoin summary: {len(summary_df)} rows -> {summary_file}")
